...
import org.victoria2.tools.vic2sgea.entities.State;
import org.victoria2.tools.vic2sgea.entities.Province;
...
public class Report {
    ...
    public Map<String, String> localizedcountries = new HashMap<>();
    public Map<String, String> localizedproducts = new HashMap<>();
    ...
    private void loadMisc(GenericObject root) {
    	...
    	String[] dates = root.getString("date").split("\\.");
    	for(int i = 1; i<3; i++)
    	{
    		if(dates[i].length() == 1)
    			dates[i] = "0"+ dates[i];
    	}
        currentDate = dates[0] + "." + dates[1] + "." + dates[2];
        playerCountry = countries.get(root.getString("player"));
        startDate = root.getString("start_date");
    }

    /**
     * Reads localizations for given game/mod path
     */
     ...
     * @throws IOException
     */
    private void readCountryNames() throws IOException {
    	InputStream str = getClass().getResourceAsStream("/localization/countries.txt");
        InputStreamReader reader = new InputStreamReader(str, "UTF-8"); // This encoding seems to work for ö and ü
        ...
        while ((line = in.readLine()) != null) {
            ...
            if (dataArray == null || dataArray.length <= 1) // <=1 - otherwise dataArray[1] gaves Out of Boundaries exception in setCountryName(dataArray[0], dataArray[1]) some times - nash
                continue;
            try {
            	this.localizedcountries.put(dataArray[1],  dataArray[0]);
                countries.get(dataArray[0]).setOfficialName(dataArray[1]);
    ...
    private void readProductNames() throws IOException {
    	InputStream str = getClass().getResourceAsStream("/localization/products.txt");
    	InputStreamReader reader = new InputStreamReader(str, "UTF-8");
    	BufferedReader in = new BufferedReader(reader);
    	String line;
    	Set<Map.Entry<String, Product>> list = productMap.entrySet();
    	while((line = in.readLine()) != null) {
    		String[] dataArray = line.split(";");
    		if (dataArray == null || dataArray.length <= 1)
    			continue;
    		this.localizedproducts.put(dataArray[1],  dataArray[0]);
    		for(Map.Entry<String, Product> p:list)
    		{
    			if(p.getValue().name.matches(dataArray[0]))
    			{
    				p.getValue().setRealName(dataArray[1]);
    				continue;
    			}
    		}
    	}
    ...
    private void loadCountry(GenericObject countryObject) {
        ...
        //정부 상태
        String government = countryObject.getString("government");
