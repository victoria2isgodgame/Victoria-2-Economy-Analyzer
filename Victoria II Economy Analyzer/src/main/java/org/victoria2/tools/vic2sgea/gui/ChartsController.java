...
/**
 * By Anton Krylov (anthony.kryloff@gmail.com) Date: 2/2/17 12:05 AM
 * <p>
...
public class ChartsController extends BaseController {
	...
	private List<GridPane> charts = new ArrayList<>();
	private int page;
	...
	protected PieChart addChart(List<PieChart.Data> pieChartData, int i, int j, String title,
		...
		Double totalValue = chart.getData().stream().map(PieChart.Data::getPieValue).reduce(0., (d1, d2) -> d1 + d2);
		...
		for (final PieChart.Data data : chart.getData()) {
			...
			data.getNode().addEventHandler(MouseEvent.MOUSE_ENTERED, e -> caption.setText(onEnter.apply(data)));
	 ...
	 * Function<PieChart.Data, String> onEnter, Consumer<PieChart.Data> onClick) {
	 * int row = (chartCount / 2); int column = chartCount % 2;
	 * 
	 * chartCount++; return addChart(pieChartData, column, row, title, onEnter,
	 * onClick);
	 * 
	 * }
	 */

	protected PieChart addChart(List<ChartSlice> slices, String title, Function<PieChart.Data, String> onEnter,
			Consumer<PieChart.Data> onClick) {
		int row = (chartCount % 6) / 3;
		int column = (chartCount % 6) % 3;

		chartCount++;

		List<PieChart.Data> pieChartData = slices.stream().map(chartSlice -> chartSlice.data)
	...
	protected void setPage(int page) {
		...
		for (int i = 0; i < 6; i++) {
			if (charts.size() > this.page * 6 + i)
				grid.getChildren().remove(charts.get(this.page * 6 + i));
			if (charts.size() > page * 6 + i)
				grid.add(charts.get(page * 6 + i), i % 3, i / 3 + 2);
		}
		this.page = page;
	}
